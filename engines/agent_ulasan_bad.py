        ...
        if tab_ulasan.count() > 0:
            ...
            try:
                page.evaluate('''() => {
                    ...
                    let targets = ["3", "2", "1"];
                    targets.forEach(t => {
                        let elements = Array.from(filterContainer.querySelectorAll('span, p, div'));
                        let match = elements.find(el => el.textContent && el.textContent.trim() === t && el.children.length === 0);
                        if (match) {
                            let wrapper = match.closest('div, label, li');
                            if (wrapper) {
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: py agent_ulasan_bad.py <url> <output_file>")
        sys.exit(1)
